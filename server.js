...
["privacidad","terminos","cookies","envios","visualizador"].forEach(p =>
  app.get(`/${p}`, (_req, res) =>
...
app.post("/api/generate-3d", async (req, res) => {
  ...
  if (!apiKey) return res.status(500).json({ error: "MESHY_API_KEY no configurada en Railway Variables." });
...
app.get("/api/poll-3d", async (req, res) => {
  ...
  try {
    ...
    if (!r.ok) return res.status(r.status).json({ error: `Meshy API error: ${r.status}` });
    ...
    if (data.status === "SUCCEEDED") {
      ...
      return res.json({
        status: "SUCCEEDED",
        glbUrl:    proxy(data.model_urls?.glb),
        objUrl:    data.model_urls?.obj ?? null,
...
app.post("/api/gallery-save", async (req, res) => {
  ...
  if (!process.env.BLOB_READ_WRITE_TOKEN)
    return res.json({ ok: true, persisted: false, message: "Galería en modo demo (sin BLOB_READ_WRITE_TOKEN)" });
  ...
  try {
    ...
    const entries = await Promise.all(
      blobs.sort((a,b) => new Date(b.uploadedAt)-new Date(a.uploadedAt))
        .slice(0, limit)
        .map(async blob => {
          try { return await (await fetch(blob.url)).json(); } catch { return null; }
        })
    );
